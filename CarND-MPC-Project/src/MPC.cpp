...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      fg[0] += 8.4*CppAD::pow(vars[cte_start + i], 2);
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      fg[0] += 17.1*CppAD::pow(vars[a_start + i], 2);
      //fg[0] += 700*CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
    }

    for (int i = 0; i < N - 2; i++) {
      fg[0] += 0.01*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // set all non-actuiators upper and lower bounds to max and min value
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] = 1.0e19;
  }

// The upper and lower bounds of delta are set to -25 and 25 degrees
